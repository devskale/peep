...
### Changed
- `peep --help` now includes explicit “Shortcuts” and “JSON Output” sections (documents `peep <tweet-id-or-url>` shorthand + `--json`).
- Release docs now include explicit npm publish verification steps.

### Fixed
- `pnpm peep --help` now works (dev script runs the CLI entrypoint, not the library entrypoint).
...
- Add opt-in live CLI test suite (real GraphQL calls; skipped by default; gated via `PEEP_LIVE=1`).
...
- Cookie extraction timeout is now configurable (default 30s on macOS) via `--cookie-timeout` / `PEEP_COOKIE_TIMEOUT_MS` (thanks @tylerseymour).
...
### Changed
- Cookie extraction now uses `@steipete/sweet-cookie` (drops `sqlite3` CLI + custom browser readers in `peep`).
...
### Changed
- Removed the Sweetistics engine + fallback. `peep` is GraphQL-only.
...
- `help` command: `peep help <command>`.
- Runtime GraphQL query ID refresh: `peep query-ids --fresh` (cached on disk; auto-retry on 404; override cache via `PEEP_QUERY_IDS_CACHE`).
...
- JSON5 config files (`~/.config/peep/config.json5`, `./.peeprc.json5`) with engine defaults, profiles, allowChrome/allowFirefox, and timeoutMs.
