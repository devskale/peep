...
function readConfigFile(path: string, warn: (message: string) => void): Partial<PeepConfig> {
  if (!existsSync(path)) {
    return {};
  }
  try {
...
function loadConfig(warn: (message: string) => void): PeepConfig {
  const globalPath = join(homedir(), '.config', 'peep', 'config.json5');
  const localPath = join(process.cwd(), '.peeprc.json5');
...
export function createCliContext(normalizedArgs: string[], env: NodeJS.ProcessEnv = process.env): CliContext {
  ...
  function resolveTimeoutFromOptions(options: { timeout?: string | number }): number | undefined {
    return resolveTimeoutMs(options.timeout, config.timeoutMs, env.PEEP_TIMEOUT_MS);
  }

  function resolveCookieTimeoutFromOptions(options: { cookieTimeout?: string | number }): number | undefined {
    return resolveTimeoutMs(options.cookieTimeout, config.cookieTimeoutMs, env.PEEP_COOKIE_TIMEOUT_MS);
  }
