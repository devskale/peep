...
export function createProgram(ctx: CliContext): Command {
  ...
  program.addHelpText(
    'beforeAll',
    () =>
      `${ctx.colors.banner('peep')} ${ctx.colors.muted(getCliVersion())} ${ctx.colors.subtitle(
        '— fast X CLI for tweeting, replying, and reading',
      )}`,
  );

  program.name('peep').description('Post tweets and replies via Twitter/X GraphQL API').version(getCliVersion());
  ...
  program.addHelpText(
    ...
    () =>
      `\n${ctx.colors.section('Examples')}\n${[
        formatExample('peep whoami', 'Show the logged-in account via GraphQL cookies'),
        formatExample('peep --firefox-profile default-release whoami', 'Use Firefox profile cookies'),
        formatExample('peep tweet "hello from peep"', 'Send a tweet'),
        formatExample(
          'peep 1234567890123456789 --json',
          'Read a tweet (ID or URL shorthand for `read`) and print JSON',
        ),
      ].join('\n\n')}\n\n${ctx.colors.section('Shortcuts')}\n${[
        formatExample('peep <tweet-id-or-url> [--json]', 'Shorthand for `peep read <tweet-id-or-url>`'),
      ...
      )}\n${ctx.colors.muted(`  (Run ${ctx.colors.command('peep <command> --help')} to see per-command flags.)`)}`,
  );
  ...
  program.addHelpText(
    ...
    () =>
      `\n\n${ctx.colors.section('Config')}\n${ctx.colors.muted(
        `  Reads ${ctx.colors.argument('~/.config/peep/config.json5')} and ${ctx.colors.argument('./.peeprc.json5')} (JSON5)`,
      )}\n${ctx.colors.muted(
        ...
        `  ${ctx.colors.option('NO_COLOR')}, ${ctx.colors.option('PEEP_TIMEOUT_MS')}, ${ctx.colors.option('PEEP_COOKIE_TIMEOUT_MS')}, ${ctx.colors.option('PEEP_QUOTE_DEPTH')}`,
      )}`,
  );
