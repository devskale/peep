...
function resolveFeaturesCachePath(): string {
  const override = process.env.PEEP_FEATURES_CACHE ?? process.env.PEEP_FEATURES_PATH;
  if (override && override.trim().length > 0) {
    return path.resolve(override.trim());
  }
  return path.join(homedir(), '.config', 'peep', DEFAULT_CACHE_FILENAME);
}
...
function readOverridesFromEnv(): NormalizedFeatureOverrides | null {
  const raw = process.env.PEEP_FEATURES_JSON;
