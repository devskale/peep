...
export function resolvePackageVersion(importMetaUrl?: string): string {
  const injected =
    typeof process !== 'undefined' && typeof process.env.PEEP_VERSION === 'string'
      ? process.env.PEEP_VERSION.trim()
      : '';
...
export function resolveGitSha(importMetaUrl?: string): string | null {
  const injected =
    typeof process !== 'undefined' && typeof process.env.PEEP_GIT_SHA === 'string'
      ? process.env.PEEP_GIT_SHA.trim()
      : '';
  if (injected.length > 0) {
