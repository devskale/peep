...
function withTempDir<T>(fn: (dir: string) => T): T {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'peep-version-test-'));
  try {
    return fn(dir);
  } finally {
...
describe('getCliVersion', () => {
  afterEach(() => {
    delete process.env.PEEP_VERSION;
    delete process.env.PEEP_GIT_SHA;
  });
  ...
  it('formats injected version + sha', () => {
    process.env.PEEP_VERSION = '9.9.9';
    process.env.PEEP_GIT_SHA = 'abcdef123456';
