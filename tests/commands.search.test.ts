...
describe('search command', () => {
  ...
  it('requires --all or --cursor when --max-pages is provided', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'peep', 'search', 'cats', '--max-pages', '2'])).rejects.toThrow(
        'exit 1',
      );
  ...
  it('requires positive --count when not paging', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'peep', 'search', 'cats', '--count', '0'])).rejects.toThrow('exit 1');
  ...
  it('requires positive --max-pages when paging', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'peep', 'search', 'cats', '--all', '--max-pages', '0'])).rejects.toThrow(
        'exit 1',
      );
  ...
  it('uses paged search when --all is set', async () => {
    ...
    try {
      await program.parseAsync(['node', 'peep', 'search', 'cats', '--all', '--json']);
  ...
  it('uses cursor pagination when --cursor is set', async () => {
    ...
    try {
      await program.parseAsync(['node', 'peep', 'search', 'cats', '--cursor', 'cursor-1']);
