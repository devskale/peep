...
describe('runtime-features', () => {
  afterEach(() => {
    delete process.env.PEEP_FEATURES_JSON;
    delete process.env.PEEP_FEATURES_CACHE;
    delete process.env.PEEP_FEATURES_PATH;
    clearFeatureOverridesCache();
  });

  it('applies global and set overrides from env json', () => {
    process.env.PEEP_FEATURES_JSON = JSON.stringify({
      global: {
        global_flag: true,
        ignored: 'nope',
      },
  ...
  it('refresh merges defaults with existing cache', async () => {
    const cacheDir = path.join(os.tmpdir(), `peep-test-${randomUUID()}`);
    ...
    process.env.PEEP_FEATURES_PATH = cachePath;
